...
SMDModel::Mesh::Mesh(const std::string &tex)
	: texture(tex)
{}
...
void SMDModel::ReadTriangleBlock(VFilePtr &f)
{
	...
	while(ReadBlockLine(f,l) == true)
	{
		if(vertId == -1)
		{
			...
			auto it = std::find_if(m_meshes.begin(),m_meshes.end(),[tex](Mesh &mesh) {
				return (mesh.texture == tex) ? true : false;
			});
			if(it == m_meshes.end())
			{
